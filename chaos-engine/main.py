...
def execute_kafka_flood():
    logger.warning("CHAOS: Flooding Kafka queue with 10k messages")
    
    def producer_thread():
        try:
            p = Producer({'bootstrap.servers': 'kafka:9092', 'message.timeout.ms': 1000})
...
def api_clear_chaos():
    ...
    # 2. Clear network shaping on all possible target containers
    targets = ["user-service-1", "user-service-2", "order-service", "notif-service"]
    for t in targets:
        try:
